...
object Vec3 {
    implicit def intToVec3(v: Int) = Vec3(v, v, v)
    implicit def doubleToVec3(v: Double) = Vec3(v, v, v)

    private val rand = new scala.util.Random

    def random = Vec3(rand.nextDouble(), rand.nextDouble(), rand.nextDouble())
...
case class Vec3(x: Double = 0, y: Double = 0, z: Double = 0) {
    ...
    def reflect(other: Vec3) = this - 2*dot(other)*other

    def length_squared = x*x + y*y + z*z
    def length = sqrt(length_squared)
    def unit_vector = this/length
